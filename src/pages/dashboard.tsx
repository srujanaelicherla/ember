    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-gradient-to-br from-pink-200 via-purple-200 to-blue-200">
        ...
        <div className="space-y-4">
          ...
          <div className="flex space-x-2">
            <input
              ...
              // text-slate-700 fixes the white text issue
              className="flex-1 p-2 rounded-lg bg-white/60 border border-white/50 text-slate-700 font-medium placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-300"
            />
            <button
              onClick={() => {
      ...
      <div className="w-full max-w-4xl space-y-4">
        {recentRooms.length > 0 && (
          <>
            <h2 className="text-xl font-bold text-slate-700 ml-2">Recent Rooms</h2>
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <button
                        ...
                        className="text-slate-400 hover:text-red-500 transition font-bold text-sm p-1"
                      >
                        ✕
                      </button>
                    </div>
                    <h3 className="font-bold text-slate-700 mt-2 truncate">{room.name}</h3>
                    <p className="text-[10px] font-mono text-slate-500 mt-1 uppercase truncate">{room.id}</p>
                    <div className="text-right mt-3">
                      <span className="text-xs font-bold text-indigo-400 group-hover:underline">Rejoin →</span>
                    </div>
                  </div>
                );
              })}
        ...
        {loadingRooms && <p className="text-center text-slate-500 text-sm animate-pulse">Checking history...</p>}
        {!loadingRooms && recentRooms.length === 0 && (
          <p className="text-center text-slate-500 text-sm italic opacity-60">No rooms saved yet.</p>
        )}
      </div>
      ...
      {roomToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/20 backdrop-blur-sm">
          <div className="w-full max-w-xs bg-white/90 backdrop-blur-2xl border border-white shadow-2xl rounded-2xl p-6 text-center animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-sm text-slate-500 mb-6 leading-relaxed">
              This will remove the room and all tasks for everyone. This cannot be undone.
            </p>
            <div className="flex gap-3">
              ...
              <button 
                ...
                className="flex-1 py-2 rounded-lg font-semibold bg-red-500 text-white hover:bg-red-600 shadow-md transition"
              >
                Delete
              </button>
            </div>
